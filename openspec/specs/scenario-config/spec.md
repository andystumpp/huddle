...
The active scenario set is composed from the built-in scenarios plus a `scenarios` section in `huddle.config.json`. Configuration can disable built-in scenarios by key and define custom scenarios inline; those custom scenarios run through the same shared trail → provider → `NudgeDraft` pipeline as the built-ins, so a config-authored scenario produces the same `Nudge` shape without touching code.
